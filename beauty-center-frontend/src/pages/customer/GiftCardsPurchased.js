...
const GiftCardsPurchased = () => {
  const { data, isLoading, error } = useQuery('giftcards-purchased', giftCardsAPI.getPurchased);
  const cards = data?.data || [];

  if (isLoading) return <div className="flex justify-center mt-12"><LoadingSpinner /></div>;
  if (error) return <div className="text-center text-red-600 mt-8">Erreur lors du chargement des cartes cadeaux achetées.</div>;

  return (
    <div className="max-w-3xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Cartes cadeaux achetées</h1>
        <Link
          to="/customer/giftcard-purchase"
      ...
      {cards.length === 0 ? (
        <div className="text-center text-gray-500">Aucune carte cadeau achetée pour le moment.</div>
      ) : (
            ...
            <tbody>
              {cards.map(card => (
                <tr key={card.id || card._id} className="border-t">
                  ...
                  <td className="px-4 py-2">{card.recipientName}<br /><span className="text-xs text-gray-500">{card.recipientEmail}</span></td>
                  ...
                  <td className="px-4 py-2">{card.purchaseDate ? new Date(card.purchaseDate).toLocaleDateString('fr-FR', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default GiftCardsPurchased; 